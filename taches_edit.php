    ...
    <div class="min-h-screen bg-[#B4CA65] flex items-center justify-center p-6">
        <form method="POST" class="w-full max-w-xl bg-white p-8 rounded-2xl shadow-lg space-y-6">
            ...
            <div>
                ...
                <input type="text" name="sujet" value="<?= clean_input($tache['sujet']) ?>" required class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-[#B4CA65]">
            </div>

            <!-- Description -->
            <div>
                <label class="block text-[#333] mb-1 font-semibold">Description</label>
                <textarea name="description" value="<?= clean_input($tache['message']) ?>" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-[#B4CA65]"></textarea>
            </div>

            <!-- Étapes -->
            <div>
                <label class="block text-[#333] mb-1 font-semibold">Étapes (si complexe)</label>
                <textarea value="<?= clean_input($tache['etapes'] ?? '') ?>" name="etapes" rows="4" class="w-full resize-none px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-[#B4CA65]"></textarea>
            </div>

            <!-- Fréquence -->
            <div>
                <label class="block text-[#333] mb-1 font-semibold">Fréquence (si récurrente)</label>
                <select name="frequence" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-[#B4CA65]">
            ...
            <div>
                ...
                <input type="date" name="date" value="<?= clean_input($tache['date_limite']) ?>" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-[#B4CA65]">
            </div>

            <!-- Statut -->
            <div>
                <label class="block text-[#333] mb-1 font-semibold">Statut</label>
                <select name="statut" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-[#B4CA65]">
